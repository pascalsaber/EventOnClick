...
function AllUserEvents() {
    ...
    useEffect(() => {
        async function main() {
            try {
                const fetchData = await fetch_URL_GET("http://localhost:5000/event/allEvents?Admin=1", token);
                                ...
                                <tbody>
                                    {data ? data.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>{item.notes}</td>
                                            <td>{item.status}</td>
                                            <td>{Object(item.payments).totalCost}</td>
                                            <td><button onClick={() => navigate(`/addEvent?eventid=${item._id}`)}>Edit</button></td>
                                        </tr>
